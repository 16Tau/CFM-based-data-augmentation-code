...
class MIFGSMAttack:
    def __init__(self, model, eps=0.03, alpha=0.001, steps=10, mu=1.0, random_start=True):
        self.model = model
        self.eps = eps
        self.alpha = alpha
        self.steps = steps
    ...
    def generate_adversarial(self, image_tensor, target_class=None):
        ...
        for step in range(self.steps):
            ...
            adv_image = adv_image.detach() + self.alpha * momentum.sign()
...
def process_image_with_mifgsm(image_path, model, transform, eps, alpha, steps, mu, thresholds, grid_size=9, sigma=2):
    ...
    mifgsm_attack = MIFGSMAttack(model, eps=eps, alpha=alpha, steps=steps, mu=mu)
    adv_tensor = mifgsm_attack.generate_adversarial(image_tensor)
...
def main():
    ...
    for img_path in image_files:
        ...
        original, adv_image = process_image_with_mifgsm(
            img_path, model, transform, eps, alpha, steps, mu, thresholds
        )
        if original is None:
            continue
        ...
        output_path = os.path.join(threshold_dir, f"{base_name}_mifgsm_brightened.png")
